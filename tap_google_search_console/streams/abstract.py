...
class BaseStream(ABC):
    ...
    def tap_stream_id(self) -> str:
        """
        The unique identifier for the stream. This is allowed to be different from the name of the stream
...
class IncrementalTableStream(BaseStream, ABC):
    ...
    def get_bookmark(state: Dict, stream: str, site: str, sub_type: str, default: str) -> str:
        """
        Fetches the bookmark from the state file for a given stream, site, sub_type
        """
    ...
    def set_dimensions_in_payload(self, stream_metadata: Dict) -> List:
        """
        Set only the selected (field selection) dimensions in API payload
        """
        selected_dimensions = []
        for dimension in self.dimension_list:
            if should_sync_field(metadata.get(stream_metadata, ("properties", dimension), "inclusion"),
    ...
    def make_payload(self, sub_type: str, start_date: str, end_date: str, stream_metadata: Dict) -> Dict:
        ...
        if sub_type in ["discover", "googleNews"]:
            ...
            if self.tap_stream_id == "performance_report_query" and "query" in self.body_params["dimensions"]:
    ...
    def process_records(self, schema: Dict, stream_metadata: Dict, records: List, time_extracted: datetime,
                        max_bookmark_value=None, last_datetime=None) -> str:
        """
        Filters out the unselected fields by the user
        Picks the latest bookmark value from extracted data
        Writes the records to stdout
        """
